...
t_log* inicializar_logger(char *nombre)
{
	...
	nuevo_logger = log_create(nombre_con_extension, "LOGGER CPU", 1, log_level_from_string(log_level()));
...
t_log* inicializar_logger(char *nombre) {
    ...
    size_t longitud = strlen(nombre) + strlen(".log") + 1;
    char* nombre_con_extension = malloc(longitud);
    printf("Longitud del nombre con extensión: %zu\n", longitud);
    ...
    printf("Creando logger con nombre: %s\n", nombre_con_extension);
    if (nuevo_logger == NULL) {
        free(nombre_con_extension);
        exit(EXIT_FAILURE);
    }
    ...
    printf("Logger creado exitosamente.\n");
    return nuevo_logger;
}
