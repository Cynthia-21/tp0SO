...
void atender_kernel_io(char* nombre_interfaz){
    ...
    while (control_key) {
	    ...
	    case PAQUETE:
			...
			cargar_int_al_buffer(respuesta, pid);
			cargar_string_al_buffer(respuesta, nombre_interfaz);

			t_paquete* paquete = crear_paquete(PAQUETE, respuesta);
