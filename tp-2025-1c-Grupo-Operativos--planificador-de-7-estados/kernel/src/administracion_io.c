...
void atender_fin_io(int socket) {
    //Me traigo el dispositivo que termino
...
void atender_desconexion_io(int socket) {
    //Me traigo el dispositivo que se desconecto
    t_dispositivo_io* dispositivo = dictionary_get(dispositivos, string_itoa(socket));
    ...
    if (dispositivo->pcb_io != NULL) {
        ...
        log_info(kernel_logger, "## (%d) - Finaliza por desconexión de IO (%s)", dispositivo->pcb_io->pcb->PID, dispositivo->nombre_io);
    }
    ...
    if(io->conectados == 0){
        ...
        dictionary_remove(ios, dispositivo->nombre_io);
    }
}
