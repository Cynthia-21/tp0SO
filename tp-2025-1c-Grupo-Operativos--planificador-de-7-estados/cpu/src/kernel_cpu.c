    ...
    while (control_key) {
        ...
        switch (cod_op) {
            ...
            if(respuesta == RESULT_OK) {
                log_info(cpu_logger, "HANDSHAKE EXITOSO BB");
            ...
            // deserializar_pc_pid(buffer, &pc, &pid);

            comenzar_ciclo_instruccion(pid, pc, cpu_logger);

            } break;
            case PAQUETE:
            log_info(cpu_logger, "Me llegaron los siguientes valores desde %d:\n", socket_kernel_dispatch);
            break;
