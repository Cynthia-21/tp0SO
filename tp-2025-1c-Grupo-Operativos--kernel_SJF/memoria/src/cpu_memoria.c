...
void handle_solicitar_instruccion(int socket_cpu){
    t_buffer* buffer = recibir_buffer(socket_cpu); //buffer tiene pid y pc
    ...
    t_instruccion* instruccion_enviar = list_get(lista_instrucciones, pc); //Obtiene la instruccion de la psosicion pc
...
void handle_leer_memoria(int socket){
    ...
    // Recibo mensaje de CPU 
    t_buffer* buffer = recibir_buffer(socket);
    char* direccion = extraer_string_del_buffer(buffer);
    ...
    cargar_string_al_buffer(buffer_rta, "Te devuelvo lo leido");
...
void handle_escribir_memoria(int socket){
    ...
    // Envio confirmación a CPU de que escribí
    t_buffer* buffer_rta = crear_buffer();
